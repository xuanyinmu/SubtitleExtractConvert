...
inline void SubtitleDecoder::RecoverOutputPathImp(const std::string& metaTitle)
{
    m_OutFileDir.resize(m_OutFileDir.size() - metaTitle.size());
    unsigned int subSize = 0;
    switch (m_SubtitleType)
    {
    case SUBTITLE_NONE:
        std::cout << "SUBTITLE_NONE" << std::endl;
        break;
    case SUBTITLE_BITMAP:
        std::cout << "SUBTITLE_BITMAP" << std::endl;
        break;
    case SUBTITLE_TEXT:
        subSize = 4;
        break;
    ...
    m_OutFileDir.resize(m_OutFileDir.size() - subSize);
}
...
inline void SubtitleDecoder::SaveSubtitleFlieImp(const AVSubtitle& subtitle, const std::string& metaTitle)
{
    ...
    std::ofstream ofs(m_OutFileDir, std::ios::out | std::ios::app);
    if (!ofs.is_open())
    {
    ...
    ofs.close();
    RecoverOutputPathImp(metaTitle);
}
...
{
    if (m_OutFileDir.size() > 4 && (CPP_ENDWITH_SRT(m_OutFileDir) || CPP_ENDWITH_ASS(m_OutFileDir)))
    {
        return true;
    }
    return false;
...
void SubtitleDecoder::FFmpegInit()
{
    ...
    const AVDictionary* metadata = nullptr;
    ...
    {
        // 查找解码器
        codecParameters = m_FormatContextPtr->streams[pair.first]->codecpar;
        ...
        */
        ...
        if (avcodec_open2(codecContext, codec, nullptr) == 0)
        {
            ...
            // 遍历并打印所有元数据键值对
            metadata = m_FormatContextPtr->streams[pair.first]->metadata;
            ...
            if (!tagValue.empty())
            {
                m_IndexCodecContextUMap[pair.first] = { codecContext, tagValue};
            }
        }
        else
        {
